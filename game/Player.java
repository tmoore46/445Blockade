...
public class Player {
    ...
    public boolean hasWon(BlockadeGUI game) {
        boolean piece0Win = false;
        boolean piece1Win = false;
        for (int[] winPositions : victoryCells) {
            if (piece0.checkWinLocation(winPositions, game.GAME_BOARD)) piece0Win = true;
            if (piece1.checkWinLocation(winPositions, game.GAME_BOARD)) piece1Win = true;
        }
        return piece0Win && piece1Win;
    }
